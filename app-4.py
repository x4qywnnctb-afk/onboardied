...
def create_chart_01_sentiment():
    """Chart 1: Sentiment Butterfly - sorted best (top) to worst (bottom)"""
    ...
    ax.set_title('Chart 1: Sentiment Analysis - What Engineers Talk About',
    ...
    plt.savefig('adyen_charts/01_sentiment_butterfly.png', dpi=DPI, bbox_inches='tight',
                facecolor=ADYEN_WHITE, edgecolor='none')

    return fig


def create_chart_02_heatmap():
    """Chart 2: Platform Heatmap - grey for missing, colour only for real data"""
    ...
    # Pivot WITHOUT filling NaN — keep missing as NaN
    ...
    # Mask = True where data is missing (will be painted grey)
    mask = pivot.isna()

    # Fill NaN with 0 only for the colour array (seaborn needs a number)
    ...
    sns.heatmap(
        ...
        linewidths=1,
        linecolor=ADYEN_BORDER,
        ax=ax,
        vmin=0, vmax=100,
        annot_kws={'fontsize': 10, 'weight': '600', 'color': ADYEN_MIDNIGHT}
    )

    # Paint masked cells grey and write "N/A"
    for row_i, row_label in enumerate(pivot.index):
        for col_i, col_label in enumerate(pivot.columns):
            if pd.isna(pivot.loc[row_label, col_label]):
                ax.add_patch(plt.Rectangle(
                    (col_i, row_i), 1, 1,
                    fill=True, color='#e8eaed', zorder=2
                ))
                ax.text(
                    col_i + 0.5, row_i + 0.5, 'N/A',
                    ha='center', va='center',
                    fontsize=9, color='#9E9E9E',
                    fontweight='normal', zorder=3
                )

    ax.set_title('Chart 2: Platform Ratings — Actual Data Only (N/A = not rated)',
                 fontsize=13, weight='600', pad=15, color=ADYEN_MIDNIGHT)
    ...
    plt.tight_layout(rect=[0, 0.06, 1, 1])
    plt.figtext(0.5, 0.01,
                'Data: Blind/Glassdoor/Comparably/Indeed (2024-2025) | N/A = platform does not rate this category | Serafima, Feb 2026',
                ha='center', fontsize=8, color=ADYEN_SECONDARY)

    os.makedirs('adyen_charts', exist_ok=True)
    plt.savefig('adyen_charts/02_platform_heatmap.png', dpi=DPI, bbox_inches='tight',
                facecolor=ADYEN_WHITE, edgecolor='none')
...
with st.sidebar:
    ...
    selected_chart = st.selectbox(
        "Select Chart",
        [
            "Chart 1: Sentiment Butterfly",
            "Chart 2: Platform Heatmap",
...
if selected_chart == "Chart 1: Sentiment Butterfly":
    st.markdown(f"""
...
elif selected_chart == "Chart 2: Platform Heatmap":
    st.markdown(f"""
        <h2 style='color: {ADYEN_MIDNIGHT}; font-size: 1.8rem; font-weight: 600; margin-bottom: 1rem;'>
            Chart 2: Platform Ratings Comparison
        </h2>
        <p style='color: {ADYEN_SECONDARY}; font-size: 1rem; margin-bottom: 1.5rem;'>
